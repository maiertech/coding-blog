...
recommend keeping a local development environement ready.
...
If plan B does not work for you, as is the case with my 2019 Macbook Air, you
can try to setup a minimal local development environment with
[Homebrew](https://brew.sh/). With Homebrew, you can install and update your
development tools, such as [Git](https://git-scm.com/),
[Node](https://nodejs.org/en/) and [Yarn](https://yarnpkg.com/) with minimal
overhead. This works on macOS, Linux and Windows Subsystem for Linux. All that
is missing are VS Code extensions defined in your `devcontainer.json` under the
`extensions` property. You need to install those manually in your local VS Code,
if you do not already sync them via VS Code settings sync.
...
the legacy Codespaces beta community) and all of them got resovled really fast:
