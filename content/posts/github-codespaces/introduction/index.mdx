...
date: 2021-01-03
description:
  This post gives an introduction to GitHub Codespaces, a service to run
  development environments as Docker containers in the cloud and access them
  with VS Code. The configs used for Codespaces can also be used to run
  development containers locally. This creates a next generation development
  experience that makes it possible to contribute to GitHub repositories without
  the need to setup a local development environment.
tags:
  - codespaces
  - docker
---
...
past few months. There were some rough edges here and there, but overall
Codespaces did not disappoint. In this post I will explain the underlying
technology and discuss the impact of Codespaces on making contributions to
GitHub repositories so much easier.
...
in their browser or a locally installed VS Code.
...
In this example there is no `Dockerfile` and the `image` property references a
Docker image on MCR. The `settings` property contains
...
`postCreateCommand` runs `yarn install` to install dependencies. Check out the
...
recurring thing because things break or get outdated and need to be fixed
...
to a powerful development container (standard containers on Codespaces come with
4 cores, 8 GB RAM and 32 GB storage).
