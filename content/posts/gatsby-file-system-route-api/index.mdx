...
that have additional information encoded in their filename and/or pathname. This
component with `{}` notation in the filename
...
results in client-only routes following this pattern: `/users/:id`. `:id` is the
...
routes. For example, you could complement
...
In this scneario, the collection route takes precedence and for everything else
the client-only route is used.
...
will handle all `/dashboard/*` routes. You can access sub-routes like this
...
and pass it into a router, if your app uses one.
