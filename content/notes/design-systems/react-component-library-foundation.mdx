...
Live Stream with Segun:

https://www.youtube.com/watch?v=I5xEc9t-HZg

- Heavy footprint of using Styled System on top of a CSS-in-JS.
- They recognize that styling approach with a ThemeProvider was a workaround for
  when CSS variables were not supported well. But now CSs variables are well
  supported and it is time to base the solution on that.
- The question is: will they maintain Styled System's API? This might be quite
  challenging!
- It would help to understand Figma, because he models Chakra's API on how Figma
  works.
- No plans to go headless
- The focus for new components has shifted into Chakra Pro. The story about the
  layouts they norrowed from Tailwind UI is unfortunte and might hurt them
  moving forward.
...
See [note on Tailwind CSS](/notes/design-systems/tailwind-css/).
