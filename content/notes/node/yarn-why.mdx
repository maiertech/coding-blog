---
title: How Yarn 1 manages conflicting package versions
description:
  How to use 'yarn why' and the yarn.lock file to understand how Yarn minimizes
  the dependecy tree.
tags:
  - node
---
...
[hoisting](https://yarnpkg.com/advanced/lexicon#hoisting), which is a staple of
[Yarn 1](https://classic.yarnpkg.com/lang/en/). [Yarn 2](https://yarnpkg.com/)'s
[Plug'n'Play](https://yarnpkg.com/features/pnp) has addressed most issues around
hoisting, making this post partially obsolete as soon as you move to Yarn 2. In
this post Yarn (without version number) refers to Yarn 1.
...
multiple times in the tree, possibly at different versions. Yarn uses hoisting
to optimize (or rather deduplicate) the dependency tree by removing as many
nodes as possible. Yarn's documentation states that "There isn't a single way to
decide how to transform the tree, and different package managers make different
tradeoffs (some optimize for package popularity, package size, highest versions,
...). For this reason, no guarantee can be made regarding the final hoisting
layout - except that packages will always be able to access the dependencies
they listed in their manifests."
...
The conclusion is that you should not expect your `node_modules` folder to have
a specific layout. As you will see later in this post, small changes in
dependencies can cause big changes in the layout of node_modules. Another thing
to keep in mind is that hoisting packages only works when package versions are
compatible in a [semantic versioning](https://semver.org/) sense, i.e. they need
to have the same major version number (but not 0).
...
inside the
[repository with which I deployed this post](https://github.com/maiertech/coding-garden):
...
Yarn tells you that it found package
...
It also tells you that it hoisted this version, which makes sense because
...
This time Yarn looks for nodes with @maiertech/gatsby-theme-pages-core in the
...
Note that `yarn.lock` does not tell you which packages were hoisted.
...
Yarn tells you that browserslist is a direct dependency of package
[gatsby](https://www.gatsbyjs.com/) and it appears a few more times in the
dependecy tree. All versions of browserslist are compatible with version 4.16.3,
which was the latest version when I created this output. Therefore, Yarn hoisted
version 4.16.3. The entry for browserslist in `yarn.lock` reveals which version
declarations it actually found, all of them compatible with 4.16.3:
...
dependencies and pinned it to version 4.16.2. This changed things quite a bit:
...
Yarn hoisted version 4.16.2, which is the version I pinned in my dependencies.
In all other instances where browserslist is further down the dependency tree
...
you should never make assumptions about the layout of `node_modules` in your
packages.
...
- Hoisting works best if you
  ...
  constriants you create for Yarn that make it hard to hoist packages.
- You can debug issues with dependencies with a combination of running
  `yarn why` on certain packages and looking at `yarn.lock`.
- Hoisting can conceal missing dependencies. Hoisted dependencies can be
  imported anywhere. If you happen to forget to declare a dependency in your
  `package.json` and then import it, it might still work when Yarn happens to
  hoist this dependency from another node in the dependency tree. But it might
  also stop working unexpectedly, when the layout of `node_modules` changes.
- Yarn always
  ...
  i.e. Yarn will always deliver a version compatible with what you specified
  when you import a dependency.
